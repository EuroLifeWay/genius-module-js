document.getElementById('generate').addEventListener('click', async () => {
  ...
  try {
    ...
    const response = await fetch(
      'https://api.quotable.kurokeita.dev/api/quotes/random', // ?limit=10
    );
    const data = await response.json();
    console.log(data);
    console.log(data.quote.content);
    document.getElementById('post').value = data.quote.content;
...
function connect() {
  ...
  socket.onmessage = event => {
    ...
    if (data.method === 'depth_update') {
      ...
      if (price) {
        ...
        document.getElementById('price').className = trendStyle;
  ...
  socket.onclose = event => {
    console.error('Disconnected from Whitebit API:', error);
